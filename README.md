...
- `docs/SPECIFICATION.md`: 目的、取得元、重複ルール、VPS方針、品質確認の仕様書。
...
`npm run refresh` は `import:best1`、`import:partners`、`update`、`affiliate` を順に実行します。
...
将来さくらVPSへ移す場合も、基本設計は `静的配信 + 定期更新ジョブ` のままにします。予約検索やスクレイピングをユーザーアクセス時に走らせず、定期ジョブで `cruises.json` / `index.html` を更新し、Nginxなどで静的ファイルを配信します。

VPS上の想定構成:

- `/var/www/cruise-finder/current`: このリポジトリの配置先。
- `/var/www/cruise-finder/current/index.html`: Nginxで配信する公開ファイル。
- `/var/www/cruise-finder/current/cruises.json`: 正データ。将来API化するときの入力元にもする。
- `npm run refresh`: 月1または週1のcron/systemd timerで実行する更新コマンド。
- `affiliate_links.csv`: サーバー上で保持してよいが、秘密情報は入れない。管理画面やDBを作るまではCSV運用に留める。
...
- フロントは `index.html` だけで動く状態を維持する。
- 更新処理はユーザー向けWebリクエストから切り離す。
- 取得元サイトへのアクセス間隔を詰めすぎない。
- 将来DBを使う場合も、まずは `cruises.json` を生成成果物として残す。
- アフィリエイトURLや手動補正データは、更新で消えないよう `affiliate_links.csv` など別ファイルから再適用する。
